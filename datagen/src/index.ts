    ...
    try {
        ...
        while (index < IDs.length) {
            ...
            responses = responses.concat(await Promise.all(IDs.slice(index, index + ITEMS_AT_A_TIME).map(async (id) => {
                return axios.get(`https://us.api.blizzard.com/data/wow/item/${id}`, {
                    headers: {
        ...
        const items: Item[] = responses.map((response: AxiosResponse) => blizzardItemToItem(response.data));
        console.log(items);

        const fs = require('fs');
        fs.writeFile("test.json", JSON.stringify(items), function (err: Error) {
            if (err) {
                console.log(err);
            }
        });
...
const blizzardItemToItem = (blizzardItem: BlizzardItem): Item => {
    return {
        id: blizzardItem.id,
